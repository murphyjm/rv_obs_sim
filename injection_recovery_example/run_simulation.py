"""
Example usage:
python run_simulation.py 10 10 1 --rverr_astro 0.4 --mstar 0.8 --nobs 40 --weather_losses 0.3 --obs_cadence 3 --time_jit 0.333 --num_sim 64 --num_cpus 4 --outdir_suffix test --seed 42
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the simulation code for specified scenario."
    )
    parser.add_argument(
        "mpsini",
        type=float,
        help="The Mp sini of the known planet in the system in Earth units.",
    )
    parser.add_argument(
        "period",
        type=float,
        help="The orbital period of the known planet in the system in days.",
    )
    parser.add_argument(
        "rverr_internal",
        type=float,
        help="The internal precision of the RV instrument in m/s.",
    )
    parser.add_argument(
        "--rverr_astro",
        type=float,
        default=0.4,
        help="The astrophysical jitter to add to the RVs in m/s.",
    )
    parser.add_argument(
        "--recoveries_fname",
        type=str,
        default=None,
        help="Path to the recoveries object if injection recovery tests have already been run.",
    )
    parser.add_argument(
        "--mstar",
        type=float,
        default=0.8,
        help="The mass of the host star in solar units.",
    )
    parser.add_argument(
        "--nobs",
        type=int,
        default=40,
        help="The number of RVs to include in the simulation.",
    )
    parser.add_argument(
        "--weather_losses",
        type=float,
        default=0.3,
        help="Simulate gaps in the observations from some fraction of weather losses.",
    )
    parser.add_argument(
        "--obs_cadence",
        type=float,
        default=3,
        help="The number of observations to obtain per orbital period of the known planet. E.g., obs_cadence = 3 for a planet with P = 10 d means take 3 RVs every 10 days.",
    )
    parser.add_argument(
        "--time_jit",
        type=float,
        default=0.333,
        help="The jitter to add to the time grid in units of days.",
    )
    parser.add_argument(
        "--num_sim",
        type=int,
        default=1000,
        help="The number of injection/recovery trials to conduct.",
    )
    parser.add_argument(
        "--num_cpus",
        type=int,
        default=os.cpu_count(),
        help="The number of CPUs to use for the RVSearch injection/recovery step.",
    )
    parser.add_argument(
        "--outdir_suffix",
        type=str,
        default="",
        help="Suffix to add to the end of the output directory where results will be saved.",
    )
    parser.add_argument(
        "--use_cps_date_grid",
        action="store_true",
        help="If used, then use the CPS date grid for generating observations. Note, may break if obs cadence and Nobs are incompatible.",
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed.")
    parser.add_argument(
        "--add_red_noise",
        action="store_true",
        help="If used, then add red noise in the form of a GP prediction at the observation dates. NOTE: Minimum working implementation right now.",
    )
...
def main():
    ...
    k_truth = radvel.utils.semi_amplitude(
        args.mpsini, args.period, star.get_mstar(), e, Msini_units="earth"
    )
    ...
    sim = SynthSim(
        star,
        rv_meas_err=args.rverr_internal,
        astro_jitter=args.rverr_astro,
        obs_bjd_start=obs_bjd_start,
    )
    ...
    keep_inds = np.random.choice(
        np.arange(len(time_grid)), size=args.nobs, replace=False
    )
    time_grid = np.sort(
        time_grid[keep_inds]
    )  # Create some gaps in the observations due to weather losses.
    ...
    tc = radvel.orbit.timeperi_to_timetrans(
        orbel[1], orbel[0], e, omega, secondary=False
    )
    priors = [
        radvel.prior.Gaussian("per1", args.period, 1e-3),
        radvel.prior.Gaussian("tc1", tc, 1e-1),
    ]
    ...
    df_mcmc = radvel.mcmc(
        post, save=True, savename=os.path.join(outdir, "baseline_chains.h5")
    )
    k1_med_baseline = np.median(df_mcmc["k1"])
    k1_err_baseline = np.mean(
        np.abs(np.quantile(df_mcmc["k1"], [0.16, 0.86]) - k1_med_baseline)
    )

    # Plot the results
    post.medparams = {"per1": args.period, "k1": round(k1_med_baseline, 2), "e1": e}
    radvel_plot = MultipanelPlot(
        post, uparams={"per1": 0, "k1": round(k1_err_baseline, 2), "e1": 0}
    )
    fig, axes = radvel_plot.plot_multipanel()
    fig.savefig(
        os.path.join(outdir, "baseline_radvel_fit.png"),
        dpi=200,
        facecolor="white",
        bbox_inches="tight",
    )
    ...
    if args.add_red_noise:
        ...
        plot_rvs(
            ...
            args.period,
            outdir,
            fname_suffix="_w_red_noise",
            red_noise_arr=red_noise,
        )
        ...
        fig, axes = radvel_plot.plot_multipanel()
        fig.savefig(
            os.path.join(outdir, "keplerian_radvel_fit_w_red_noise.png"),
            dpi=200,
            facecolor="white",
            bbox_inches="tight",
        )
    ...
    if args.recoveries_fname is None:
        ...
        search_obj = rvsearch.Search(
            df_rv, post=post, mcmc=False, min_per=plim[0], max_per=plim[1]
        )  # What to do if RVSearch says there is more than one signal? Maybe doesn't matter for now
        try:
            search_obj.run_search(outdir=outdir)
        except (
            ValueError
        ):  # RVSearch runs into a RadVel plotting error if there are no planets detected
        ...
        recoveries_obj = rvsearch.inject.Injections(
            pickle_fname, plim, klim, (0.0, 0.0), num_sim=args.num_sim, seed=args.seed
        )
        ...
        comp = rvsearch.inject.Completeness(
            df_recover, "inj_period", "inj_k", mstar=args.mstar
        )
        ...
        fig.savefig(
            os.path.join(outdir, "completeness.png"),
            dpi=200,
            facecolor="white",
            bbox_inches="tight",
        )
    ...
    recover_test_inds = np.random.choice(
        df_recover[df_recover.recovered == False].index,
        size=n_recover_test,
        replace=False,
    )
    ...
    orbel = (
        df_recover.loc[ind, "inj_period"],
        df_recover.loc[ind, "inj_tp"],
        0,
        np.pi / 2,
        df_recover.loc[ind, "inj_k"],
    )
    planet = RVPlanet(pl_letter="c", orbel=orbel)
    sim.star.add_planet(planet)
    df = sim.get_simulated_obs(
        time_grid
    )  # Add the planet to the time series data, but then get rid of it so that RadVel only fits a 1-planet model
    ...
    for ind in tqdm(recover_test_inds[1:]):
        ...
        if args.add_red_noise:
            plot_rvs(
                ...
                args.mpsini,
                args.period,
                recovery_test_plotting_outdir,
                fname_suffix=f"_inj_period_{orbel[0]:.2f}_injk_{orbel[-1]:.2f}",
                red_noise_arr=red_noise,
            )
        else:
            plot_rvs(
        ...
        fig.savefig(
            os.path.join(
                recovery_test_plotting_outdir,
                f"radvel_fit_inj_period_{orbel[0]:.2f}_injk_{orbel[-1]:.2f}.png",
            ),
            dpi=200,
            facecolor="white",
            bbox_inches="tight",
    ...
    df_recover_test["k1_fit_over_k1_map_baseline"] = (
        df_recover_test["k1"] / k1_map_baseline
    )
    df_recover_test["k1_fit_over_k1_med_baseline"] = (
        df_recover_test["k1"] / k1_med_baseline
    )
    df_recover_test["k1_fit_minus_k1_med_baseline_over_k1_err_baseline"] = (
        df_recover_test["k1"] - k1_med_baseline
    ) / k1_err_baseline
    df_recover_test.to_csv(
        os.path.join(outdir, "recover_test_results.csv"), index=False
    )

    # Plot the results of running the recoveries test
    fig, ax = plot_recovery_test_results(
        df_recover_fname,
        df_recover_test,
        outdir,
        mstar=args.mstar,
